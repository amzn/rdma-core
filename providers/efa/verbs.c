...
static int efa_everbs_get_ah(struct efa_context *ctx, struct efa_ah *ah,
			     struct ibv_pd *pd, struct ibv_ah_attr *attr)
{
	...
	if (write(ctx->everbs_cmd_fd, &cmd, sizeof(cmd)) != sizeof(cmd))
		return -errno;
...
struct ibv_ah *efa_create_ah(struct ibv_pd *ibvpd, struct ibv_ah_attr *attr)
{
	...
	if (ctx->cmds_supp_udata_mask & EFA_USER_CMDS_SUPP_UDATA_CREATE_AH) {
		...
		err = efa_everbs_get_ah(ctx, ah, ibvpd, attr);
		if (err) {
